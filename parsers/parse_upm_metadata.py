#!/usr/bin/env python
# coding: utf-8
''' Read UPM produced JSON for metadata
    usage: parse_upm_meta.py [-h] [--config CONFIG] [--debug] [-v] --dry_run json_files
...
def main():
    ...
    for file in args.json_files:
        ...
        with open(file) as json_file:
            ...
            for doc in data:
                logging.debug(f"Processing {doc['procurement_id']}")
                if doc['procurement_id'] not in processed_docs:
                    col = place_cols[nu.get_group(doc['procurement_id'])]
                    ref_doc = ntp.NtpEntry()
                    ref_doc.load_from_db(col, doc['procurement_id'])
                    if ref_doc.is_obsolete():
                        active_doc_id = nu.get_active_version(ref_doc.data['id'], col)
                        logging.warning(f"Document {doc['procurement_id']} is obsolete, active version is {active_doc_id}")
                        ref_doc.load_from_db(col, active_doc_id)
                        if not ref_doc:
                            logging.error(f"Active version {active_doc_id} not found")
                            continue
                    else:
                        logging.info(f"Document {doc['procurement_id']} is active")
                    processed_docs[doc['procurement_id']] = ref_doc.ntp_id
                else:
                    logging.debug(f"Document {doc['procurement_id']} already found as {processed_docs[doc['procurement_id']]}")
                    ref_doc = ntp.NtpEntry()
                    ref_doc.load_from_db(col, processed_docs[doc['procurement_id']])

                ref_doc.data['nextp_enriched/extracted_metadata'] = doc
                logging.debug(f"Document {ref_doc.ntp_id} to update ")
                logging.debug(ref_doc.data['nextp_enriched/extracted_metadata'])
                if not args.dry_run:
