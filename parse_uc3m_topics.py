    ...
    for file in args.json_files:
        ...
        with open(file) as json_file:
            ...
            for doc_place_id in data['info_docs']:
                ...
                ref_doc.data['nextp_enriched/topics'] = {}
                for topic, prob in data['info_docs'][doc_place_id].items():
                    ref_doc.data['nextp_enriched/topics'][topics[topic]] = float(prob)
                logging.debug(f"Document {ref_doc.ntp_id} to update ")
                logging.debug(ref_doc.data['nextp_enriched/topics'])
                if not args.dry_run:
